...
def getBoardScore(curGamePos):
    gamePosition=curGamePos.board
...
def AI(curGamePos, depth):
    if(depth==0):
        return curGamePos, getBoardScore(curGamePos)
    player=curGamePos.player
    boardPos=curGamePos.board
    ...
    if(player%2==0): 
        ...
        for i in range(0, 8):
            for j in range(0, 8):
                if(boardPos[i][j]!=0 and boardPos[i][j][0]=='w'):
                    f=findLegalPath(curGamePos, i, j, boardPos[i][j])
                    for fx,fy in f:
                        dupGamePos=curGamePos.clone()
                        makeMove(dupGamePos, i, j, fx, fy)
                        x, y = AI(dupGamePos, depth-1)
                        if(score<y):
                            newPos=dupGamePos
                            score=y
        return newPos, score
        ...
        for i in range(0, 8):
            for j in range(0, 8):
                if(boardPos[i][j]!=0 and boardPos[i][j][0]=='b'):
                    f=findLegalPath(curGamePos, i, j, boardPos[i][j])
                    for fx,fy in f:
                        dupGamePos=curGamePos.clone()
                        makeMove(dupGamePos, i, j, fx, fy)
    ...
    if(downClick): 
        ...
        if(peice!=0 and peice[0]==peiceChance[currentGame.player%2]):
            ...
            if((fx, fy) in legalPath):
                ...
                currentGame, score = AI(currentGame, 3)
        legalPath=[]
