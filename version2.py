...
def isCheck(gamePos, col):
    ...
    for i in range(0, 8):
        for j in range(0, 8):
            if(gamePos[i][j]==col+'K'):
                x=i
                y=j
            if(gamePos[i][j]!=0 and gamePos[i][j][0]!=col):
                attackedSquare.update(findLegalPath(gamePos, i, j, gamePos[i][j]))

    # print((x, y), attackedSquare)
    if((x, y) in attackedSquare):
        return True;
    return False

def isCheckMate(gamePos, col):
    pass
...
while(run):
    ...
    if(dragPeice!=0 and dragPeice[0]==peiceChance[turnCount%2]):
        ...
        if (finalX, finalY) in legalPath:
            ...
            if(isCheck(gamePosition, 'w')):
                print("White King under Attack")
            if(isCheck(gamePosition, 'b')):
